...
- Python 3.11+
- 本地可访问 MySQL（3306）和 Redis（6379）
  - 若没有本地实例，用 Docker Compose 只拉起基础服务：
    ```bash
    cd deploy
    docker compose up -d mysql redis
    ```
- `parquet/` 目录下放好三个 parquet 数据文件（从 OSS 或同事处获取）

### 一键启动

```bash
cd backend
cp .env.example .env   # 首次使用，按实际填写
bash dev-bootstrap.sh
```

脚本执行顺序：
1. 创建 / 激活 Python venv（`backend/.venv`）
2. 安装 / 更新依赖（`requirements.txt`）
3. 检查 `.env` 是否存在
4. 检查 MySQL / Redis 端口可达性
5. 运行 `python -m scripts.init_data`（alembic 建表 + parquet 软链就绪）
6. 启动 `uvicorn`（热重载）
...
# 6. parquet 数据就绪检查
#    - 若 backend/parquet/ 下有数据文件，自动复制并建软链
#    - 否则从 OSS 拉取（需配置 OSS 环境变量）
python -m scripts.init_data --skip-alembic
...
> **注意**：后续重启时步骤 5、6 可跳过（数据已就绪），直接运行步骤 7。  
> 若 parquet 有更新，重新执行步骤 6（加 `--force` 强制重新拉取）。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/matrix/query` | 竞争矩阵主查询 |
| POST | `/matrix/tooltip` | 矩阵单格详情 |
| POST | `/pipeline/query` | 研发泳道查询 |
| GET  | `/export/list` | 导出任务列表 |
| GET  | `/export/download/{id}` | 下载导出文件 |
