...
[oss_sync.run_sync(force)]
        ...
        │   └─ 计算每个文件的 MD5
        │
        ├─ _atomic_replace_parquet()
        │   └─ 先写 .tmp 文件，再 os.replace() 原子覆盖到 backend/parquet/
        │
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `version` | varchar | 同步版本，格式 `YYYYMMDD`（UTC 当日），唯一键 |
...
同一天多次触发时走 `ON DUPLICATE KEY UPDATE`，覆盖为最新状态。
